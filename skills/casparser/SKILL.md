...
description: >
  ...
  frontend SDK, and an MCP server with 22 live API tools.
---
...
- **ARN** — AMFI Registration Number. Identifies a Mutual Fund Distributor (MFD).
- **EUIN** — Employee Unique Identification Number. The individual advising/selling under a distributor's ARN.
- **RIA** — Registered Investment Adviser. A SEBI-registered, fee-charging adviser (registration number `INA…`). Distinct from an MFD.
- **MFD** — Mutual Fund Distributor. AMFI-registered (holds an ARN), earns commission. Distinct from a fee-only RIA.
- **KYD** — Know Your Distributor. AMFI's compliance check for distributors (surfaced as `kyd_compliant`).
...
### Pattern 10: Registry Verification (SEBI + AMFI)

Confirm an advisor or distributor is genuinely registered **before onboarding them** — a yes/no check against the official public registers. Useful when you're building for RIAs and MFDs.

- **`POST /v1/verify/sebi`** — any SEBI intermediary. Send `registration_number` and the category is auto-detected from its prefix (`INA`→investment-adviser, `INH`→research-analyst, `INP`→portfolio-manager, `INM`→merchant-banker, `INR`→registrar-transfer-agent, `INZ`→stock-broker). Send `name` + `type` to search by name; pass `type` explicitly for fund/FPI numbers that don't encode a category.
- **`POST /v1/verify/mfd`** — an AMFI distributor by `arn`, additionally screened against AMFI's suspended / terminated / terminated-EUIN lists (a hit reports `SUSPENDED`/`TERMINATED`, not just `NOT_FOUND`).

`verified` is the boolean to gate on. A "not found" is a successful `200` (`verified: false`) and is billed; only upstream failures return `5xx`. **0.25 credits** per successful verification.
...
d = r.json()
print(d["verified"], d["registration_status"], d["name"])
```

See: [`templates/python-verify.py`](templates/python-verify.py), [`references/registry-verification.md`](references/registry-verification.md), [Registry Verification Guide](https://casparser.in/docs/guides/verify)
...
| `checkKycPanStatus` | `POST /v1/kyc/pan/status` | KYC status for a PAN across all five KRAs | `pan_no` |
| `digilockerAccountLookup` | `POST /v1/kyc/digilocker/account-lookup` | Pre-consent DigiLocker registration check | `mobile` or `aadhaar_number` |
| `digilockerResult` | `POST /v1/kyc/digilocker/result/{session_id}` | Aggregate identity + issued/parsed documents | `session_id`, `fetch_documents` |
| `verifySebiIntermediary` | `POST /v1/verify/sebi` | Verify a SEBI-registered intermediary (category auto-detected) | `registration_number`, or `name` + `type` |
| `verifyMfdDistributor` | `POST /v1/verify/mfd` | Verify an AMFI distributor by ARN + adverse-list screen | `arn` |
...
- [`references/registry-verification.md`](references/registry-verification.md) — Verify SEBI intermediaries & AMFI distributors (categories, detection, adverse lists, fields)
