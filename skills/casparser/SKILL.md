...
A live MCP server at `https://cas-parser.stlmcp.com/mcp` exposes all API endpoints as tools. This lets AI agents directly call the CAS Parser API.

To use the MCP server, agents still need a valid API key. The sandbox key `sandbox-with-json-responses` works for development.
