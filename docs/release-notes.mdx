...
Verify that a financial advisor or distributor is genuinely registered — before you onboard them.

- `POST /v1/verify/sebi` — verify any SEBI-registered intermediary (Investment Adviser, Research Analyst, Portfolio Manager, Stock Broker, Merchant Banker, RTA, Mutual Fund, AIF, and more). The category is **auto-detected** from the registration number's sequence; pass `type` to search by name instead.
- `POST /v1/verify/mfd` — verify an AMFI Mutual Fund Distributor by ARN, screened against AMFI's suspended, terminated, and terminated-EUIN lists.

Both return a single `verified` boolean to gate onboarding on, plus a normalized `registration_status` (`ACTIVE`, `EXPIRED`, `SUSPENDED`, `TERMINATED`, `NOT_FOUND`). A "not found" is a successful `200` with `verified: false`.

**Credits:** 0.25 per successful verification. Failed lookups are free.

**Start here:** [Registry Verification Guide](/guides/verify)

---

## 📅 July 2026

### Email Inbox Import — Outlook and Zoho

`POST /v4/inbox/connect` now supports Microsoft Outlook and Zoho Mail in addition to Gmail. Pass `provider` in the request body to choose the mail service.

- `gmail` (default) — `@gmail.com` and Google Workspace domains
...
The resolved `provider` is returned in the connect response and reported by `POST /v4/inbox/status`. The rest of the flow (list CAS files, download, parse, disconnect) is unchanged. Billing stays at 0.2 credits per inbox pull.

**Start here:** [Email Inbox Import Guide](/guides/gmail-inbox)

---

### CAS Generator — `cas_authority: "cams"` (Beta)

`POST /v4/generate` now accepts a `cas_authority` parameter to select which RTA portal drives the mailback.

- `kfintech` (default, GA) — retrieves mutual fund data from both RTAs via the KFintech–CAMS data-sharing partnership
...
The default behaviour is unchanged. Only switch to `cams` when you specifically need the statement sourced from the CAMS portal.

**Start here:** [CAS Generator Guide](/guides/cas-generator)
...
Consent-based flow to fetch an investor's government-issued documents (Aadhaar, PAN, driving licence) directly from DigiLocker. A separate feature from KYC PAN Status.

- `POST /v1/kyc/digilocker/account-lookup` — pre-consent check (free)
- `POST /v1/kyc/digilocker/session` — start a consent session, get `authorization_url` (free)
- `POST /v1/kyc/digilocker/result/{session_id}` — one-call aggregate: verified identity, issued documents, and parsed PAN/Aadhaar/driving licence
- Explicit `consent` + `consent_purpose` captured as a consent receipt (DPDP / RBI)

**Credits:** account-lookup and session are free; `result` is 0.25 per call.

**Start here:** [KYC DigiLocker Guide](/guides/kyc-digilocker)
...
Programmatically request CAS statements via email mailback. KFintech and CAMS have a mutual partnership for data sharing and consolidation, so this retrieves mutual fund data from both RTAs.
...
| **Generation** | `POST /v4/generate` | Request CAS via email (KFintech + CAMS) |
