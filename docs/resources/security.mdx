...
For email inbox import (Gmail, Outlook, Zoho):

| Feature | Details |
|---------|---------|
| **Scope** | Read-only (cannot send emails) |
| **OAuth** | Standard OAuth 2.0 via the provider (Google, Microsoft, Zoho) |
| **Token Storage** | Encrypted server-side |
| **Revocation** | User can revoke via `/v4/inbox/disconnect` or their provider's account settings |
